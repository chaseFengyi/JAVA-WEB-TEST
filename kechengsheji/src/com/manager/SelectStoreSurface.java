...
public class SelectStoreSurface extends JFrame {
		JFrame jf = new JFrame("书库查询操作界面");
		...
		public SelectStoreSurface(){
			...
			for(int i=0; i<Integer.parseInt(data.get("总数").toString());i++){
				sign = Integer.parseInt(data.get("书库号"+(i+1)).toString());
				storage.add(sign);
			}
			fill.addAll(storage);
					...
					if( e.getSource()!= null){
						...
						for(int j = 0; j<count; j++){
							if(Integer.parseInt(i.toString()) == fill.get(j)){
								source = get.findStore(fill.get(j));
								Object[][] results = new Object[Integer.parseInt(source.get("总数").toString())][11];
								for(int k=0; k<results.length; k++){
									results[k][0] = (source.get("书库号")).toString();
									results[k][1] = (source.get("书库名")).toString();
									t = t+1;
								}
